...
const DEFAULT_TITLE_IMAGE = '../images/skynet-home.png';
...
/**
 * Ensures config has openLinksInNewTab (default true for backward compatibility).
 */
function ensureOpenLinksInNewTab(config) {
  if (config && !('openLinksInNewTab' in config)) {
    return { ...config, openLinksInNewTab: true };
  }
  return config;
}
...
function ensureSearchHistory(config) {
  if (config && !('searchHistory' in config)) {
    return { ...config, searchHistory: true };
  }
  return config;
}
  ...
  return {
    ...
    dropdowns: config.dropdowns.map((dd) => ({
      ...
      items: (dd.items || []).map((it) => {
        ...
        const obj = { value: it.value, label: it.label };
        if (Array.isArray(it.tags) && it.tags.length) obj.tags = it.tags;
        return obj;
      }),
    })),
  };
...
function migrateIfNeeded(data) {
  ...
  if (data.profiles && Array.isArray(data.profiles)) {
    const profiles = data.profiles.map((p) => ({
      ...p,
      config: ensureSearchHistory(ensureSearchBookmarks(ensureOpenLinksInNewTab(ensureDropdownItems(ensureTabSets(ensureTitleImage(p.config)))))),
      todos: Array.isArray(p.todos) ? p.todos : [],
    }));
    return { ...data, profiles };
  }
  ...
  if (data.columns && data.navbar && data.dropdowns) {
    ...
    return {
      ...
      profiles: [{
        id,
        name: 'Default',
        config: ensureSearchHistory(ensureSearchBookmarks(ensureOpenLinksInNewTab(ensureDropdownItems(ensureTabSets(ensureTitleImage(data)))))),
      }],
    };
  }
...
export function exportConfig(appData, resourceSources) {
      ...
      reader.onload = (ev) => {
        try {
          ...
          if (data.profiles && Array.isArray(data.profiles) && data.activeProfileId) {
            ...
            appData._importedResourceSources = resourceSources || null;
